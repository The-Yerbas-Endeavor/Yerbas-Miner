...
{
    if (work.size > static_cast<std::size_t>(std::numeric_limits<int>::max())) throw std::invalid_argument("GhostRider work buffer is too large");
    ...
    for (int i = 0; i < 18; ++i) {
        ...
        int cn_selection = -1;
        if (i < 5) core_selection = core_hash_indexes[i];
        else if (i == 5) cn_selection = random_cns[0];
        else if (i < 11) core_selection = core_hash_indexes[i - 1];
        else if (i == 11) cn_selection = random_cns[1];
        else if (i < 17) core_selection = core_hash_indexes[i - 2];
        else cn_selection = random_cns[2];
