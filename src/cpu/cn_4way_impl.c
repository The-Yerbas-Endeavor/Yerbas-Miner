                                     ...
                                     uint64_t tweak2, uint64_t tweak3)
{
    ...
    for (i = 0; i < YERBAS_CN_FAST_ITERATIONS; ++i) {
...

        {
            ...
            d2[0]=a20; d2[1]=a21^tweak2;
            d3[0]=a30; d3[1]=a31^tweak3;
            a00^=t00; a01^=t01; a10^=t10; a11^=t11;
            a20^=t20; a21^=t21; a30^=t30; a31^=t31;
            ax0=_mm_set_epi64x((long long)a01,(long long)a00);
            ax1=_mm_set_epi64x((long long)a11,(long long)a10);
            ax2=_mm_set_epi64x((long long)a21,(long long)a20);
            ax3=_mm_set_epi64x((long long)a31,(long long)a30);
            bx0=cx0; bx1=cx1; bx2=cx2; bx3=cx3;
        }
    }
    *ax0p=ax0; *bx0p=bx0; *ax1p=ax1; *bx1p=bx1;
    *ax2p=ax2; *bx2p=bx2; *ax3p=ax3; *bx3p=bx3;
}
#endif
                             ...
                             uint32_t iterations, size_t aes_rounds)
{
    ...
    if (yerbas_runtime_has_aes_avx2()) {
        ...
        if (page_size == 2097152U && iterations == YERBAS_CN_FAST_ITERATIONS && aes_rounds == 131072U) {
            yerbas_cn_fast_quad_loop(&ax0,&bx0,&ax1,&bx1,&ax2,&bx2,&ax3,&bx3,
                                     g_yerbas_cn_2way_scratchpad0,g_yerbas_cn_2way_scratchpad1,
                                     g_yerbas_cn_4way_scratchpad2,g_yerbas_cn_4way_scratchpad3,
