...
Hash256 sha256(const std::uint8_t* data, std::size_t size) {
    ...
    std::size_t offset = 0;
    while (size - offset >= 64) { compress(state, data + offset); offset += 64; }
    std::uint8_t tail[128]{};
    const std::size_t remain = size - offset;
    if (remain) std::memcpy(tail, data + offset, remain);
    tail[remain] = 0x80;
    const std::size_t pad_block = (remain < 56) ? 64 : 128;
    const std::uint64_t bits = static_cast<std::uint64_t>(size) * 8;
    for (int i = 0; i < 8; ++i) tail[pad_block - 1 - i] = std::uint8_t(bits >> (i * 8));
    compress(state, tail);
    if (pad_block == 128) compress(state, tail + 64);
    Hash256 out{};
