...
def type_cc_parameter(signature: str, function_name: str, stem: str) -> str:
    ...
    if context_type is None:
        return signature
    return re.sub(r"\bvoid\s*\*\s*cc\b", f"{context_type} *cc", signature, count=1)
...
def device_endian_helpers() -> str:
    return r'''/* generated CUDA endian/memory helpers */
...
__device__ __forceinline__ void yerbas_cuda_enc16le(void *dst, unsigned val)
{
    unsigned char *p = static_cast<unsigned char *>(dst);
    p[0] = (unsigned char)val; p[1] = (unsigned char)(val >> 8);
}
...
__device__ __forceinline__ void *yerbas_cuda_memmove(void *dst, const void *src, size_t len)
{
...
#define sph_enc16le yerbas_cuda_enc16le
...
#define memmove yerbas_cuda_memmove
...
def inline_helpers(source: str, specs: list[str], all_specs: list[str]) -> str:
    for spec in specs:
        ...
        source = marker.sub(f"/* begin generated inline {include_name} */\n{helper}\n/* end generated inline {include_name} */", source, count=1)
    for spec in all_specs:
        include_name, helper = load_helper(spec)
        marker = re.compile(rf'(?m)^\s*#\s*include\s+"{re.escape(include_name)}"\s*$')
        if not marker.search(source):
            raise ValueError(f"include {include_name!r} not found in source")
        source = marker.sub(lambda _m: f"/* begin generated inline {include_name} */\n{helper}\n/* end generated inline {include_name} */", source)
    return source
...
def transform_source(source: str, stem: str, inline_specs: list[str], inline_all_specs: list[str]) -> str:
    ...
    source = re.sub(rf'(?m)^void([ \t\r\n]+)(sph_{re.escape(stem)}[A-Za-z0-9_]*)', r'__device__ static void\1\2', source)

    if stem == "whirlpool":
        source = re.sub(r'(?m)^void\s*\n(SPH_XCAT\(sph_,\s*HASH\)\s*\()', r'__device__ static void\n\1', source)
        source = re.sub(r'(?m)^void\s*\\\s*$', r'__device__ static void \\', source)

    source = type_public_definitions(source, stem)
    source = re.sub(r'(?m)^(\s*)out\s*=\s*dst\s*;', r'\1out = static_cast<unsigned char *>(dst);', source)
    source = re.sub(r'(?m)^(\s*)sc\s*=\s*cc\s*;', r'\1sc = static_cast<decltype(sc)>(cc);', source)
    if stem == "simd":
    ...
    if stem in ("echo", "hamsi", "shabal"):
        source = re.sub(
            ...
            r'\1(\2, static_cast<const unsigned char *>(data),', source)
